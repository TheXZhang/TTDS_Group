...
#input("Enter the directory of your txt file:")
#text_dir = "/afs/inf.ed.ac.uk/user/s16/s1604556/ttds/trec.5000.txt"
#input("Enter the directory of your stopword file:")
...
#     #open the target txt file,and read the content by lines
#     f=open('trec.5000.txt').readlines()
#     #for each line we use regular expression "\W+" to split by words
#     tokenized_lines=[re.split('\W+',line) for line in f]
#
#     #save tokenized text to a file
#     with open('tokenized text.txt', 'w') as f:
...
def __main__():
    ...
    data=pd.read_csv("RAW_recipes.csv", names = columns)
    minutes_list=data.minutes.tolist()
    print(minutes_list)
    # date_list=data.submitted.tolist()
